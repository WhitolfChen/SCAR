...
class EfficientViT_CIFAR(torch.nn.Module):
    def __init__(self, img_size=32,
                 ...
                 window_size=[4, 4, 4],
                 kernels=[3, 3, 3, 3],
                 down_ops=[['subsample', 2], [''], ['']],
                 distillation=False,):
        super().__init__()

        resolution = img_size
        self.patch_embed = torch.nn.Sequential(
            Conv2d_BN(in_chans, embed_dim[0] // 2, 3, 2, 1, resolution=resolution), torch.nn.ReLU(),
            Conv2d_BN(embed_dim[0] // 2, embed_dim[0], 3, 1, 1, resolution=resolution // 2)
        )
...
def efficientvit_cifar():
    return EfficientViT_CIFAR()

def efficientvit_imagenet():
    return EfficientViT(num_classes=50)